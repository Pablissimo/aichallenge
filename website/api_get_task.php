...
if ($compile_result) {
    while ($row = mysqli_fetch_assoc($compile_result)) {
        if (contest_query("update_submission_compiling",
            ...
            api_log(sprintf("Error updating submission for compile task: %s", mysqli_error($mysqli)));
        }
    }
} else {
    api_log(sprintf("Error selecting next compile: %s", mysqli_error($mysqli)));
}
...
while ((!$match_result or mysqli_num_rows($match_result) == 0)
    ...
    if ($match_result and mysqli_num_rows($match_result) != 0) {
        ...
        if (!$lock_result or mysqli_affected_rows($mysqli) < 1) {
            $match_result = null;
            continue;
        }
    } else {
...
if ($match_row) {
    ...
    if ($player_result) {
        ...
        api_log(sprintf("Error selecting matchup players: %s", mysqli_error($mysqli)));
    }
} else {
    api_log(sprintf("Error selecting next match: %s", mysqli_error($mysqli)));
}
