...
if ($compiledata->status_id == 40) {
    if (contest_query("update_submission_success",
        ...
        if (!contest_query("update_submission_latest",
                          $compiledata->submission_id,
                          $compiledata->submission_id)) {
            api_log(sprintf("Error updating latest flag: %s", mysqli_error($mysqli)));
        }
        if (!contest_query("update_user_shutdown_date", $compiledata->submission_id)) {
            api_log(sprintf("Error updating shutdown date: %s", mysqli_error($mysqli)));
        }
    } else {
        api_log(sprintf("Error updating successful compile: %s", mysqli_error($mysqli)));
    }
} else {
    if (contest_query("update_submission_failure",
        ...
        api_log(sprintf("Error updating errored compile: %s", mysqli_error($mysqli)));
    }
}
